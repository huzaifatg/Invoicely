...
const SignUp = () => {
  ...
  const handleSubmit = async () => {
    // validate all fields before submissions
    const nameError = validateName(formData.name);
    const emailError = validateEmail(formData.email);
    const passwordError = validatePassword(formData.password);
    const confirmPasswordError = validateConfirmPassword(formData.confirmPassword, formData.password);

    if (nameError || emailError || passwordError || confirmPasswordError || !agreedToTerms) {
      setFieldErrors({
        name: nameError,
        email: emailError,
      ...
      // Show terms error only if user tries to submit without agreeing
      if (!agreedToTerms) {
    ...
    <div className="min-h-screen bg-white flex items-center justify-center px-4 py-8">
      <div className="w-full max-w-sm">
        ...
        <div className="space-y-4">
          ...
          {showTermsError && (
            <p className="text-sm text-red-600">You must agree to the terms and conditions to create an account</p>
          )}
